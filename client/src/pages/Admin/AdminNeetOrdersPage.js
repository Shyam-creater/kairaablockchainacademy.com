...
import { useGetAdminNeetOrdersSummaryQuery, useGetAdminNeetOrdersQuery } from "../../redux/features/admin/adminApi";
import Loader from "../../components/Loader/Loader";
import { format } from "timeago.js";
import { FiShoppingCart, FiDollarSign, FiTrendingUp, FiRefreshCw, FiEye } from "react-icons/fi";
import { motion } from "framer-motion";
import NeetOrder360Drawer from "../../components/Admin/NeetOrder360Drawer";
...
const AdminNeetOrdersPage = () => {
  const [page, setPage] = useState(0);
  const pageSize = 15;
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [selectedOrderRow, setSelectedOrderRow] = useState(null);

  const { data: summaryData, isLoading: summaryLoading, refetch: refetchSummary } = useGetAdminNeetOrdersSummaryQuery(undefined, { refetchOnMountOrArgChange: true });
  const { data: ordersData, isLoading: ordersLoading, refetch: refetchOrders } = useGetAdminNeetOrdersQuery({ page: page + 1, limit: pageSize }, { refetchOnMountOrArgChange: true });

  const summary = summaryData?.summary;
  const orders = ordersData?.purchases || [];
  const totalOrders = ordersData?.total || 0;

  const columns = [
    { field: "id", headerName: "Purchase ID", flex: 0.5, minWidth: 100, renderCell: (params) => <span className="font-mono text-slate-400 text-xs">{params.value.slice(-8)}</span> },
    { 
      field: "user", 
    ...
    { 
      ...
      renderCell: (params) => (
        <div className="flex flex-col justify-center h-full">
          <span className="text-sm text-slate-300 truncate w-full" title={params.row.fileTitle}>{params.row.fileTitle}</span>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold uppercase">{params.row.subject}</span>
            <span className="text-[10px] bg-white/10 text-white px-1.5 py-0.5 rounded font-bold">{params.row.year}</span>
          </div>
        </div>
      )
    },
  ...
  const rows = orders.map((o) => ({
    ...
    userEmail: o.userId?.email || o.userId,
    fileId: o.fileId,
  ...
  return (
    <AdminLayout
      title="NEET Purchases"
      subtitle="View purchases for NEET question banks"
      action={
        <button
      ...
      <div className="space-y-8">
        ...
        {summaryLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <KPICard
              label="Total Revenue"
              value={`₹${((summary?.revenue ?? 0) / 100).toLocaleString("en-IN", { maximumFractionDigits: 0 })}`}
              sub={`₹${((summary?.last30Days?.revenue ?? 0) / 100).toLocaleString("en-IN", { maximumFractionDigits: 0 })} last 30 days`}
              iconKey="revenue"
              delay={0.2}
            />
            <KPICard
              label="Avg. Purchase"
              value={`₹${((summary?.avgOrderValue ?? 0) / 100).toLocaleString("en-IN", { maximumFractionDigits: 0 })}`}
...
export default AdminNeetOrdersPage;
