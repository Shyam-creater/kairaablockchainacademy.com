  ...
  <div className="flex gap-1 p-1 bg-white/5 border border-white/10 rounded-xl w-fit mb-6">
    {tabs.map((t, i) => (
      <button
        key={i}
        onClick={() => onChange(i)}
        className={`px-6 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${active === i
                    ...
                    {isLoading ? (
                      ...
                      assignments.map((a, i) => (
                        <div
                          ...
                          className={`relative z-10 pl-14 pr-4 py-3 rounded-xl cursor-pointer transition-all border ${activeAssignment?._id === a._id
                        ...
                        >
                          {/* Timeline dot */}
                          <div className={`absolute left-[13px] top-1/2 -translate-y-1/2 w-[11px] h-[11px] rounded-full border-2 border-[#0B0F19] ${a.status === 'pending' ? 'bg-warning shadow-[0_0_8px_rgba(255,179,0,0.6)]' :
