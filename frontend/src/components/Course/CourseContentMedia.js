...
import { useAddNewQuestionMutation,useAddAnswerInQuestionMutation } from '../../redux/features/courses/coursesApi.js';
  ...
  return (
    <div className='w-[95%] 800px:w-[86%] py-4 m-auto'>
      ...
      <div className="w-full p-4 flex items-center justify-between bg-slate-500 bg-opacity-20 backdrop-blur shadow-[bg-slate-700] rounded shadow-inner">
        {["Overview", "Resources", "Q&A"].map((text, index) => (
          <h5
            key={index}
            className={`800px:text-[20px] mx-4 cursor-pointer ${styles.button} ${
              activeBar === index
                ? "text-red-500"
                : " text-white"
            }`}
