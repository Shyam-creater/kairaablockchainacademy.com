...
export const getUserOrders = CatchAsyncError(async (req, res, next) => {
  ...
  const ordersWithCourseDetails = await Promise.all(orders.map(async (order) => {
    const course = await Course.findById(order.courseId).select('name price estimatedPrice thumbnail');
    return {
      ...order.toObject(),
      course
    };
  ...
  res.status(200).json({
    success: true,
    orders: ordersWithCourseDetails,
  });
});
