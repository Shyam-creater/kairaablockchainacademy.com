...
export const uploadMeetingRecording = CatchAsyncError(async (req, res, next) => {
  try {
    ...
    if (materialFile) {
      ...
      // Notify targeted students about the recording/materials
      const Notification = (await import("../models/notificationModel.js")).default;
      const sendMail = (await import("../utils/sendMail.js")).default;
      const { User } = await import("../models/userModel.js");
      ...
      if (meeting.batchId) {
        const Batch = (await import("../models/batchModel.js")).default;
        const batch = await Batch.findById(meeting.batchId).populate("students", "name email");
        if (batch) targetStudents = batch.students;
      } else {
        const orders = await Order.find({ assignedStaffId: req.user._id, courseId: meeting.courseId }).populate("userId", "name email");
        targetStudents = orders.map(o => o.userId).filter(u => u != null);
      }

      const notifMessage = `A recording or new material has been uploaded for the meeting: '${meeting.topic}'.`;
      ...
      for (const student of targetStudents) {
        ...
        if (student.email) {
          sendMail({
            email: student.email,
            subject: "Meeting Recording Available - Kairaa Academy",
            template: "notification.ejs",
            data: { user: { name: student.name }, title: "Meeting Recording Available", message: notifMessage, dashboardUrl: process.env.CLIENT_URL }
          }).catch(err => console.error("Failed to send recording email:", err));
        }
      }
...
export const createProjectTask = CatchAsyncError(async (req, res, next) => {
  try {
    ...
    // Notify targeted students about the new project
    const Notification = (await import("../models/notificationModel.js")).default;
    const sendMail = (await import("../utils/sendMail.js")).default;
    const Order = (await import("../models/orderModel.js")).default;
    let targetStudents = [];
    ...
    const message = `A new project '${title}' has been posted. Due date: ${new Date(dueDate).toLocaleDateString()}.`;
    ...
    for (const student of targetStudents) {
      ...
      if (student.email) {
        sendMail({
          email: student.email,
          subject: "New Project - Kairaa Academy",
          template: "notification.ejs",
          data: { user: { name: student.name }, title: "New Project Posted", message, dashboardUrl: process.env.CLIENT_URL }
...
export const staffReviewProject = CatchAsyncError(async (req, res, next) => {
  try {
    ...
    // Notify student
    const student = await (await import("../models/userModel.js")).User.findById(project.studentId);
    if (student) {
      const Notification = (await import("../models/notificationModel.js")).default;
      const sendMail = (await import("../utils/sendMail.js")).default;
      
      const notificationMessage = `Your project '${project.title}' has been reviewed. Status: ${status}.`;
      ...
      if (student.email) {
        sendMail({
          email: student.email,
          subject: "Project Reviewed - Kairaa Academy",
          template: "notification.ejs",
          data: { user: { name: student.name }, title: "Project Reviewed", message: notificationMessage, dashboardUrl: process.env.CLIENT_URL }
        }).catch(err => console.error("Failed to send review email:", err));
      }
    }
