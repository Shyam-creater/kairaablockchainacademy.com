  ...
  try {
    ...
    if (orders.length > 0) {
      ...
      if (latestCourse) {
        ...
        totalLessonsInCurrentCourse = latestCourse.courseData?.length || latestCourse.courseContentData?.length || 0;
      }
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        overallProgress: totalLessonsInCurrentCourse > 0 ? Math.min(100, Math.round((totalCompletedLessons / totalLessonsInCurrentCourse) * 100)) : 0,
