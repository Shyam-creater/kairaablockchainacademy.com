...
export const generateCertificateQR = async (certificateNumber) => {
  try {
    const verifyUrl = `${process.env.FRONTEND_URL || "https://kairaablockchainacademy.com"}/verify-certificate/${certificateNumber}`;
...
export const generateCertificatePDF = async (certificateData) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc.text(`Verify at: ${process.env.FRONTEND_URL || "https://kairaablockchainacademy.com"}/verify-certificate/${certificateData.certificateNumber}`, 50, doc.page.height - 65);
