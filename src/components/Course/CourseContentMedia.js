...
const CourseContentMedia = ({ data, id, activeVideo, setActiveVideo, user, refetch, completedLessons = [], onMarkWatched, isCurrentLessonCompleted, courseDetails, meetings, activeCourseTab = 6, isEmbedded = false, hideVideo = false, pendingRecordingUrl = null, pendingRecordingTitle = null, onRecordingPlayed }) => {
  ...
  const isEmbeddableVideo = (url) => {
    ...
    if (lowerUrl.endsWith('.mp4') || lowerUrl.endsWith('.webm') || lowerUrl.endsWith('.ogg')) return true;
    return false;
  };
    ...
    <div className='w-full flex flex-col xl:flex-row gap-6'>
      ...
      {activeBar === 6 && (
        <div className="w-full mt-4 animate-fade-in">
          <AICoPilot lessonTitle={data[activeVideo]?.title || "this lesson"} />
        </div>
      )}
